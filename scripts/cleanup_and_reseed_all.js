...
async function seedDonationClaims(donations, recipients) {
  ...
  try {
    ...
    const claimsByMode = {
      ...
      direct: Math.min(3, donationsByMode.direct.length),
  ...
  try {
    ...
    for (let i = 0; i < shuffledClaims.length && deliveriesToInsert.length < numDeliveries; i++) {
      ...
      if (status === 'delivered') {
        ...
        delivery.volunteer_notes = 'Delivery completed successfully. Recipient was very grateful!';
      }
...
async function seedDirectDeliveries(claims, donations, recipients) {
  ...
  try {
    ...
    // Create direct deliveries for 50-70% of direct claims
    const numDirectDeliveries = Math.min(
      Math.floor(directClaims.length * 0.6),
      directClaims.length
    );
    
    for (let i = 0; i < numDirectDeliveries; i++) {
      const claim = directClaims[Math.floor(Math.random() * directClaims.length)];
      
      // Skip if this claim already has a direct delivery
      if (directDeliveriesToInsert.some(d => d.claim_id === claim.id)) {
        continue;
      }
